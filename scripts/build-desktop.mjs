...
for (const name of ['main', 'preload']) await build({
  entryPoints: [join(root, 'desktop', `${name}.ts`)], bundle: true, platform: 'node',
  format: 'cjs', target: 'node22', external: ['electron'], outfile: join(out, `${name}.cjs`),
});
await build({ entryPoints: [join(root, 'src/main/server.ts')], bundle: true, platform: 'node',
  format: 'cjs', target: 'node22', outfile: join(out, 'backend.cjs'),
...
cpSync(join(root, 'src/infrastructure/windows/collect-snapshot.ps1'), join(assets, 'collect-snapshot.ps1'));
if (existsSync(join(root, 'sidecar/bin'))) cpSync(join(root, 'sidecar/bin'), assets, { recursive: true });
...
writeFileSync(join(out, 'package.json'), JSON.stringify({ name: 'kadroskop', productName: 'Кадроскоп', version: '0.1.0', main: 'main.cjs' }, null, 2));
