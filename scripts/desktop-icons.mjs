...
export function desktopIcon(recording = false) {
  ...
  const points = [[5,16],[10,16],[13,8],[17,24],[21,16],[27,16]];
  for (let i = 1; i < points.length; i++) {
    const [x0,y0] = points[i-1], [x1,y1] = points[i];
    const steps = Math.max(Math.abs(x1-x0),Math.abs(y1-y0));
    for (let t=0;t<=steps;t++) {
      const x=Math.round(x0+(x1-x0)*t/steps), y=Math.round(y0+(y1-y0)*t/steps);
      plot(x,y,[176,232,208]); plot(x+1,y,[176,232,208]);
    }
  }
  if (recording) for(let y=1;y<12;y++) for(let x=21;x<32;x++) if((x-26)**2+(y-6)**2<25) plot(x,y,[255,95,86]);
