...
if ((await ensurePresentMon()) === null)
  throw new Error('Не удалось подготовить PresentMon для сборки.');
...
if (!existsSync(join(application, 'assets/PresentMon.exe')))
  throw new Error('В сборке отсутствует PresentMon.');
writeFileSync(
  join(out, 'ЧИТАЙ.txt'),
  'Запустите Кадроскоп.exe. Крестик сворачивает окно в трей. Выход — через меню значка рядом с часами.\nЗаписи хранятся в %APPDATA%/Кадроскоп/sessions и сохраняются при обновлении программы.\nДля переноса старых записей скопируйте содержимое прежней папки sessions сюда при закрытом приложении.\n',
  'utf8',
);
