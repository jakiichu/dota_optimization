...
import type {
  SessionRecord,
  SessionStore,
} from '../../src/application/ports/session-store.port.ts';
    ...
    load: vi.fn(async (id: string): Promise<SessionRecord> => {
      ...
      return {
        ...
        capture: {
          ...
          frames: frameTrace(Array<number>(60000 / ms).fill(ms)),
        },
        sensors: [],
...
describe('повторное сравнение сохранённых записей', () => {
  it('принимает группы через маршрут и пересчитывает сырые кадры', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      query.append('before', `before-${i}`);
      query.append('after', `after-${i}`);
    }
    ...
    expect(result.pairs.map((pair) => pair.before.id)).toEqual([
      'before-0',
      'before-1',
      'before-2',
    ]);
  ...
  it('отвергает повторяющиеся записи до чтения больших файлов', async () => {
    const store = makeStore();
    await expect(
      new CompareSessions(store).executeRepeated(['a', 'b', 'c'], ['a', 'd', 'e']),
