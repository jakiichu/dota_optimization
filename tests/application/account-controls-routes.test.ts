...
describe('маршруты переноса управления', () => {
  it('отдают найденные профили', async () => {
    ...
    const route = createAccountControlsRoutes(store).find(
      (item) => item.path === '/api/account-controls',
    )!;
  ...
  it('передают только выбранные идентификаторы в сценарий', async () => {
    const transfer = vi.fn(async () => ({
      source: profile,
      target: { ...profile, id: '202', label: 'Второй' },
      backupPath: null,
      transferredBytes: 42,
    }));
    const store: AccountControlsStore = { list: async () => [], transfer };
    const route = createAccountControlsRoutes(store).find((item) =>
      item.path.endsWith('/transfer'),
    )!;
  ...
  it('передаёт полный режим и отклоняет неизвестный', async () => {
    ...
    const route = createAccountControlsRoutes({ list: async () => [], transfer }).find((item) =>
      item.path.endsWith('/transfer'),
    )!;
    await route.handle(
      new URLSearchParams(),
      JSON.stringify({ sourceId: '101', targetId: '202', mode: 'all' }),
    );
    expect(transfer).toHaveBeenCalledWith('101', '202', 'all');
    await expect(
      route.handle(
        new URLSearchParams(),
        JSON.stringify({ sourceId: '101', targetId: '202', mode: 'invalid' }),
      ),
