...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'kadroskop-transfer-'));
  roots.push(root);
  const source = join(root, 'source');
  const target = join(root, 'target');
  await mkdir(join(source, 'remote'), { recursive: true });
  await mkdir(join(target, 'remote'), { recursive: true });
...
describe('целостность переноса', () => {
  ...
  it('откатывает удаление нового файла при ошибке восстановления следующего', async () => {
    const { source, target } = await fixture();
    const files = ['remote/user_keys.vcfg', 'remote/user_convars.vcfg'];
    await writeFile(join(source, files[1]!), 'old');
    for (const file of files) await writeFile(join(target, file), 'current');
    failure.target = join(target, files[1]!);
    await expect(
      transferSettings(source, target, files, { kind: 'restore' }, [files[0]!]),
    ).rejects.toThrow('Изменения отменены');
    for (const file of files) expect(await readFile(join(target, file), 'utf8')).toBe('current');
  });
  ...
  it('не начинает запись при ошибке чтения следующего исходного файла', async () => {
    ...
    await expect(
      transferSettings(source, target, ['remote/user_keys.vcfg', 'remote/user_convars.vcfg']),
    ).rejects.toThrow();
