...
describe('шаги проверки настройки', () => {
  ...
  it('после сохранения предлагает повторную запись, не подтверждая применение в игре', () => {
    expect(
      experimentProgress(experimentHypothesis(), [{ name: 'FPS_MAX', value: '60' }], []),
    ).toMatchObject({ step: 'capture', saved: true });
  });
  ...
  it('учитывает последнее значение повторяющейся переменной', () => {
    expect(
      experimentProgress(
        ...
        [
          { name: 'fps_max', value: '60' },
          { name: 'fps_max', value: '120' },
        ],
        [],
      ).saved,
    ).toBe(false);
  ...
  it('предлагает только совместимые записи после начала проверки', () => {
    const hypothesis = {
      ...experimentHypothesis(),
      candidates: [
        { id: 'old', comparable: true },
        { id: 'new', comparable: true },
        { id: 'different', comparable: false },
        { id: 'missing', comparable: true },
      ],
    };
    const sessions = [
      experimentSession('old', '2026-09-11T10:30:00Z'),
      experimentSession('new', '2026-09-11T12:00:00Z'),
      experimentSession('different', '2026-09-11T12:30:00Z'),
    ];
    expect(experimentProgress(hypothesis, [], sessions)).toMatchObject({
      step: 'compare',
      candidates: ['new'],
    });
  });
  ...
  it('не продолжает проверку без исходной записи', () => {
    expect(experimentProgress({ ...experimentHypothesis(), before: null }, [], []).step).toBe(
      'missing-baseline',
    );
  });
});
