...
export type HypothesisOutcome =
  ...
  | 'refuted'
  /** Изменение не прошло выбранные пороги; эффект по этой паре не обнаружен. */
  | 'no-change'
...
function describe(
  ...
  if (outcome === 'no-change') {
    return (
      `Не подтвердилась: ${numbers} — изменение не превышает выбранные пороги. ` +
      'По этой паре записей эффект не обнаружен. Повторите замеры в тех же условиях.'
    );
  }
