...
/**
 * Как назвать влияние настройки, когда речь о ней одной.
 *
 * Формулировка осторожная намеренно. Мы знаем, какой части движка настройка
 * касается, — но не знаем, сколько она стоит **на этой машине**: это
 * выясняется замером, а не чтением таблицы. Утверждать «даёт кадры» по одному
 * лишь имени переменной значило бы делать ровно то, за что мы ругаем конфиги
 * из интернета.
 *
 * Поэтому здесь «возможно», а точный ответ даёт раздел «Запись кадров».
 */
export const IMPACT_LABEL: Record<CvarImpact, string> = {
  gpu: 'возможно, разгружает видеокарту',
  cpu: 'возможно, разгружает процессор',
  both: 'возможно, разгружает процессор и видеокарту',
  gameplay: 'меняет саму игру, а не её скорость',
  cosmetic: 'только внешний вид',
  none: 'на скорость не влияет',
};

/** Короткая пометка для перечислений: «по видеокарте: 24, по процессору: 9». */
export const IMPACT_SHORT: Record<CvarImpact, string> = {
  gpu: 'по видеокарте',
  cpu: 'по процессору',
  both: 'по процессору и видеокарте',
  gameplay: 'про саму игру',
  cosmetic: 'про внешний вид',
