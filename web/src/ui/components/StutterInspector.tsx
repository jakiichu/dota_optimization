...
export function StutterInspector({
  ...
  const selected = stutters.find((entry) => entry.stutter.frameIndex === selectedFrameIndex) ?? stutters[0]!;
  ...
  return <div className="card">
    ...
    <ol className="stutter-inspector-list" aria-label="Самые длинные статтеры">
      {stutters.map((entry, index) => <li key={entry.stutter.frameIndex}>
        <button type="button" className="stutter-inspector-item"
    ...
    <div className="stutter-inspector-detail" aria-live="polite">
      ...
      {selected.evidence.length === 0 ? <p>Совпавших событий не найдено.</p> : <ul>
        {selected.evidence.map((evidence) => <li key={`${evidence.kind}:${evidence.detail}`}>
      ...
      {selected.relatedRecommendation !== null && selected.recommendationAnchor !== null && (
        <div className="stutter-next-check">
          ...
          <span className="muted">Рекомендация рассчитана по всей записи, а не по одному кадру.</span>
        </div>
      )}
      <p className="muted">Эти события совпали с рывком во времени. Совпадение помогает выбрать следующую проверку, но само по себе не доказывает причину.</p>
    </div>
  </div>;
}

function Metric({ label, value }: { label: string; value: string }): React.JSX.Element {
  return <div className="metric"><span className="metric-label">{label}</span><span className="metric-value">{value}</span></div>;
}
