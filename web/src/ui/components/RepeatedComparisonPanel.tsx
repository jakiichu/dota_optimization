...
export function RepeatedComparisonPanel({
  sessions,
}: {
  ...
  const update = (side: 'before' | 'after', index: number, id: string): void => {
    (side === 'before' ? setBefore : setAfter)((ids) =>
      ids.map((old, i) => (i === index ? id : old)),
    );
  };
  ...
  return (
    <div className="card">
      ...
      <p className="muted">
        Запишите один участок повтора трижды с исходной настройкой. Измените одну настройку,
        примените её в игре и запишите тот же участок ещё трижды. Сохраняйте длительность, камеру и
        фоновые программы. Выберите записи в порядке прогонов.
      </p>
      {sessions.length < 6 && (
        <p className="muted">
          Для проверки нужны шесть разных записей. Сейчас доступно: {sessions.length}.
        </p>
      )}
      {before.map((_, index) => (
          ...
          {(['before', 'after'] as const).map((side) => (
            <label key={side}>
              <span className="metric-label">
                {side === 'before' ? 'до' : 'после'} · прогон {index + 1}
              </span>
              <select
                className="input"
                value={(side === 'before' ? before : after)[index]}
                ...
                {sessions.map((session) => (
                  <option
                    ...
                    disabled={
                      ...
                      (side === 'before' ? before : after)[index] !== session.id
                    }
                  >
      ...
      {duplicate && (
        <p className="notice error">Каждая запись должна использоваться только один раз.</p>
      )}
      ...
      {result.data !== undefined && (
        <>
          <p
            ...
            style={{
              color:
                result.data.status === 'inconclusive' || result.data.status === 'not-comparable'
                  ? 'var(--muted)'
                  : VERDICT_COLOR[result.data.status],
            }}
          >
            {result.data.summary}
          </p>
          <table className="table">
            <thead>
              <tr>
                <th>метрика</th>
                <th>до · медиана [мин–макс]</th>
                <th>после · медиана [мин–макс]</th>
              </tr>
            </thead>
            <tbody>
              {result.data.metrics.map((metric) => (
                <tr key={metric.id}>
                  ...
                  {[metric.before, metric.after].map((values, i) => (
                    <td key={i}>
                      {withUnit(values.median, metric.unit)} [{values.min.toFixed(1)}–
          ...
          <details>
            <summary>Результаты каждой пары</summary>
            <ol>
              {result.data.pairs.map((pair) => (
                <li key={pair.before.id}>
                  {pair.before.label} → {pair.after.label}: {pair.summary}
                </li>
              ))}
            </ol>
          </details>
