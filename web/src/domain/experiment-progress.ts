...
export function experimentProgress(
  ...
  const values = new Map(
    settings?.map((setting) => [setting.name.toLowerCase(), setting.value.trim()]),
  );
  const saved =
    settings !== undefined &&
    hypothesis.recommendation.changes.length > 0 &&
    hypothesis.recommendation.changes.every(
      (change) => values.get(change.cvar.toLowerCase()) === change.value.trim(),
    );
  ...
  const candidates = hypothesis.candidates
    .filter((candidate) => {
      ...
      return (
        candidate.comparable && session !== undefined && session.capturedAt > hypothesis.createdAt
      );
    })
